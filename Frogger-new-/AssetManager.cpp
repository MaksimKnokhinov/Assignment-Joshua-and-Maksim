...
namespace SDLFramework {
	...
	TTF_Font* AssetManager::GetFont(std::string filename, int size) {
		std::string fullPath = SDL_GetBasePath(); //string which will return the directory where your project is saved
		fullPath.append("Assets/" + filename);
	...
	SDL_Texture* AssetManager::GetText(std::string text, std::string filename, int size, SDL_Color color, bool managed) {
		std::stringstream ss;
		ss << size << (int)color.r << (int)color.g << (int)color.b; //setting text colors
