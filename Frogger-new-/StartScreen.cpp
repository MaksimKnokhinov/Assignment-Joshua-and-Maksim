...
StartScreen::StartScreen() {
	...
	m_pKonami = new Texture("Konami", "namco__.ttf", 36, { 200,0,0 });
...
void StartScreen::FrogTitleAnimStage()
{
	if (mAninmationTitleStage == 0) {
		...
		mAnimationStartPos = Vector2(Graphics::SCREEN_WIDTH+400, Graphics::SCREEN_HEIGHT * 1.0f);
	 ...
	 if(mAninmationTitleStage==1){
		 ...
		 if (!t_bool2) {
			 mAnimationTimer = 0;
			 mAnimationStartPos = Vector2(Graphics::SCREEN_WIDTH * .5f, Graphics::SCREEN_HEIGHT * 1.0f);

			 mAnimationEndPos = Vector2(Graphics::SCREEN_WIDTH * .5f, Graphics::SCREEN_HEIGHT * 0.5f);
			 t_bool2 = true;
				
		 }//a ct_b	heck to make onlyfireonce
...
void StartScreen::Update() {
	...
	if (!mAnimationDone) {
		FrogTitleAnimStage();	
		mAnimationTimer += m_pTimer->DeltaTime()*.5f;
